...
class x2h_env extends uvm_scoreboard;
    ...
    axi2ahb_virtual_sequencer axi2ahb_virtual_sequencer_h;
...
function void x2h_env::build_phase(uvm_phase phase);
  ...
  ahb_env_h = ahb_env::type_id::create("ahb_env_h",this);
    ...
    axi2ahb_virtual_sequencer_h = axi2ahb_virtual_sequencer::type_id::create("axi2ahb_virtual_sequencer_h",this);
...
function void x2h_env::connect_phase(uvm_phase phase);
  ...
  if(axi4_env_cfg_h.has_virtual_seqr) begin
    foreach(axi4_master_agent_h[i]) begin
      axi2ahb_virtual_sequencer_h.axi4_master_write_seqr_h = axi4_env_h.axi4_master_agent_h[i].axi4_master_write_seqr_h;
      axi2ahb_virtual_sequencer_h.axi4_master_read_seqr_h = axi4_env_h.axi4_master_agent_h[i].axi4_master_read_seqr_h;
    end
  end
  ...
  if(ahb_env_config.has_virtual_seqr) begin
    foreach(ahb_slave_agent_h[i]) begin
      axi2ahb_virtual_sequencer_h.ahb_slave_sequencer_h = ahb_env_h.ahb_slave_agent_h[i].ahb_slave_sequencer_h;
    end
  end
  foreach(ahb_slave_agent_h[i]) begin
