module axi2fifo(
	...
	// Capture control information write for AW chanel when write finished burst and fifo not full
	always@(posedge aclk or negedge aresetn)
		if(!aresetn)begin
			awid_reg <= 8'b0;
			awaddr_reg <= 32'b0;
	...
	// Calculate warp boundary and check approach boundary for Wrap burst
	always@(posedge aclk or negedge aresetn)
		if(!aresetn)begin
	...
	// Count the beats master send to  
	always@(posedge aclk or negedge aresetn)
		if(!aresetn)
			awlen_temp <= 8'b0;
		else if(wlast == 1'b1 && fifo_access == 1'b0)
	...
	// Flag for response of bridge arrive master in AW, W chanel
	...
	// Capture value from master for AR chanel
	always@(posedge aclk or negedge aresetn)
		if(!aresetn)begin
			arid_reg <= 8'b0;
			araddr_reg <= 32'b0;
	...
	// Calculator for WRAP BURST  in AR chanel
	always@(posedge aclk or negedge aresetn)
		if(!aresetn)begin
	...
	// Flag signal arready depend on fifo and read_finish privious transaction
	assign arready = (read_finished == 1'b0||fifo_access == 1'b1)?1'b0:1'b1;

	//state machine control state for 
	always@(posedge aclk or negedge aresetn)
		if(!aresetn)
			cstate <= IDLE;
		else
